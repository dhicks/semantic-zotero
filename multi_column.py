...
def column_boxes(page, footer_margin=50, header_margin=50, no_image_text=True):
    ...
    ## If there's an excessive number of blocks, skip processing this page
    if len(blocks) > 1000:
        return []
    ...
    # DJH: This is incorrect: when boxes don't run the entire length of column, we end up bouncing between columns. Sort by column first (left) and then top to bottom
